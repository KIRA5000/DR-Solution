...
class CDynamicVHDx : public CBaseDisk
{
  ...
  public:
    ...
    CDynamicVHDx(const std::string& path) : CBaseDisk(path, _1M)
    {
    ...
    {
      std::cout << "\n Chunk Ratio : " << iChunkRatio << " iTotalBatEntries : " << iTotalBatEntries << "\n";
      ...
      {
        char *region_type = "Unknown Region";
      ...
      for (int i = 0; i < metadata_table_hdr->EntryCount; i++)
      {
        char *entry_type = "Unknown Metadata Table Entry";
  ...
  protected:
    ...
    virtual size_t DataBlockRead(DataBlockIO& f) override
    {
      ...
      {
        auto fRet = CSubject::ReadSync(f.buffer, f.length, ((((VHDX_BAT_ENTRY *) &batentry)->FileOffsetMB) * _1M) + blockoff);
    ...
    virtual size_t DataBlockWrite(DataBlockIO& f) override
    {
      ...
      if (state == VHDX_BAT_ENTRY_PAYLOAD_BLOCK_NOT_PRESENT)
      {
        ...
        auto fRet = CSubject::WriteSync(iPayloadBlock.get(), GetBlockSize(), (((VHDX_BAT_ENTRY *) &batentry)->FileOffsetMB) * _1M);
